...
mongoose.connect(
  "mongodb+srv://krishnatripathy2001:Krishna25@curd.s8xsq8c.mongodb.net/curd",
...
app.post("/signup", async (req, res) => {
  try {
    ...
    if (role === "admin") {
      ...
      res
        .status(201)
        .json({ message: "User and patient created successfully" });
    }
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.delete("/deleteUser/:id", (req, res) => {
  const userIdFromClient = req.params.id;
  console.log("Deleting user with ID:", userIdFromClient);

  User.findByIdAndDelete(userIdFromClient)
    .then((result) => {
      if (!result) {
